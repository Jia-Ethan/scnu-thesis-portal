...
- 首页上传入口不再暴露不可见键盘焦点，也不会绕过冲突保护
- 编号参考文献、注释区编号项和明显列表项不会被正文标题识别掏空
- 首个识别到的二级 / 三级 / 四级标题不会再导出 `0.*` 前导零编号
...
- 首页上传入口已改为“可见按钮 -> 显式 `input.click()`”链路，隐藏 file input 已移出 tab 焦点链
- `SOURCE_CONFLICT`、busy、exporting 三种状态都会阻止实际选文件
- `detect_heading(...)` 的数字标题推断已收紧：`参考文献`、`注释`、`附录`、`目录` 区块中的编号条目不会再被正文标题规则直接吞走
- 首个正文标题从二级 / 三级 / 四级起步时，导出编号分别回填为 `1.1.`、`1.1.1.`、`1.1.1.1.`
- 英文摘要关键词前缀已兼容 `Keyword:`、`Keywords:`、`Key words:`，且关键词行不会残留在摘要正文中
