...
## Forma frontend entry

当前公开首页正在收敛为 **Forma**：一个通用的 AI 论文格式优化入口。

主流程调整为两步输入：

1. 粘贴学校、学院或期刊发布的论文格式要求。
2. 上传 `.docx` 论文文件。

首页只保留核心入口；使用说明已移动到前端 Guide 页（`/#/guide`）。`.docx` 文件校验、公开预检与 Word 导出复用现有 API；格式要求解析、Agent 自动修复策略与差异报告目前仍是前端流程预留，后续应接入真实格式修复 Agent / ruleset API 后再标记为正式能力。
